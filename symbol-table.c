...
void addSymbol(const char *name, const char *type, const char *scope, int used) {
...
int main() {
        ...
        while (token != NULL) {
            if (strcmp(token, "int") == 0) {
                ...
                if (token != NULL && isalpha(token[0])) {
                    addSymbol(token, "int", scopeStack[scopeDepth], 0);
                }
    ...
    printf("Scope | Symbol Name | Type | Address | Used\n");
    printf("------------------------------------------------------\n");
    for (int i = 0; i < symbolCount; i++) {
        printf("%s | %s | %s | %d | %s\n",
               symbolTable[i].scope,
